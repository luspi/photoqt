function(composeDesktopFile)
    ...
    set(GMMIME "${GMMIME};image/jpeg;application/x-mif;video/x-mng;image/x-portable-bitmap;image/x-photo-cd;image/vnd.zbrush.pcx;image/x-portable-graymap;")
    set(GMMIME "${GMMIME};image/x-xpixmap;image/x-portable-pixmap;image/x-portable-anymap;image/x-pict;image/png;audio/vnd.dts.hd;text/x-mpsub;image/rle;")
    set(GMMIME "${GMMIME};image/x-sgi;image/x-sun-raster;image/x-tga;image/tiff;image/vnd.wap.wbmp;image/webp;application/x-wpg;image/x-xbitmap;")
    set(GMMIME "${GMMIME};image/x-xpixmap;image/x-xwindowdump")
    if(GM)
        foreach(MIME ${GMMIME})
            list(FIND COMBINEDMIMETYPE "${MIME}" FOUNDPOS)
            if(${FOUNDPOS} MATCHES -1)
    ...
    set(DEVILMIME "${DEVILMIME};image/gif;image/jpeg;image/x-photo-cd;image/png;image/x-portable-bitmap;image/x-portable-graymap;image/x-portable-pixmap;")
    set(DEVILMIME "${DEVILMIME};image/x-portable-anymap;image/vnd.adobe.photoshop;image/x-panasonic-rw;image/x-sgi;image/x-tga;image/tiff")
    if(DEVIL)
    ...
    set(FREEMIME "${FREEMIME};image/x-photo-cd;video/x-mng;image/vnd.zbrush.pcx;image/x-portable-bitmap;image/x-portable-graymap;image/x-portable-pixmap;")
    set(FREEMIME "${FREEMIME};image/x-portable-anymap;image/png;image/x-pict;image/vnd.adobe.photoshop;image/x-sun-raster;image/x-sgi;image/x-tga;image/tiff;")
    set(FREEMIME "${FREEMIME};image/vnd.wap.wbmp;image/webp;image/x-xbitmap;image/x-xpixmap")
    if(FREEIMAGE)
