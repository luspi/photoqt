...
ZipReader::FileInfo QZipPrivate::fillFileInfo(int index) const
{
    ...
    while (filePathRef.endsWith(QLatin1Char('/')))
        filePathRef = filePathRef.left(filePathRef.size()-1);
...
*/
QVector<ZipReader::FileInfo> ZipReader::fileInfoList() const
{
    d->scanFiles();
