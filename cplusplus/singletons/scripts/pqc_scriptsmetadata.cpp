...
QString PQCScriptsMetaData::analyzeExposureTime(const QString val) {
    ...
    // if we get either a single value (all good) or more than two parts (confusing), then do nothing
    if(parts.length() != 2)
        return val;
    ...
    const int t1 = parts[0].toInt(&ok1);
    const int t2 = (parts[1].contains(' ') ? parts[1].split(' ').first().toInt(&ok2) : parts[1].toInt(&ok2));

    // invalid data
    if(!ok1 || !ok2 || t1 == 0.0 || t2 == 0.0)
        return "0";
...
QString PQCScriptsMetaData::analyzeFlash(const QString val) {
    ...
    bool ok = false;
    int _val = val.toInt(&ok);
    if(!ok) {
        qWarning() << "converting" << val << "to int failed.";
        return val;
    }
...
QString PQCScriptsMetaData::_analyzeGPSHelper(QString pos) {

    QStringList split = pos.split(" ");
...
QPointF PQCScriptsMetaData::convertGPSToDecimal(QString gpsLatRef, QString gpsLat, QString gpsLonRef, QString gpsLon) {
    ...
    QStringList lat = gpsLat.split(" ");
    QStringList lon = gpsLon.split(" ");
    ...
    if(lon.length() == 1)
        ...
        lon << "0";
    else if(lon.length() != 3)
        return QPointF(9999,9999);

    double x = 0, y = 0;

    const double div[] = {1, 60, 3600};

    for(int i = 0; i < 3; ++i) {
        x += _convertGPSToDecimalHelper(lat.at(i))/div[i];
        y += _convertGPSToDecimalHelper(lon.at(i))/div[i];
    }
...
double PQCScriptsMetaData::_convertGPSToDecimalHelper(QString pos) {
