...
QString PQCScriptsShortcuts::getTranslation(QString key) {
    const QString key_check = key.toLower().trimmed();
    if(keyStrings.contains(key_check))
        return keyStrings[key_check];
    if(mouseStrings.contains(key_check))
        return mouseStrings[key_check];
    return key;
}
...
QString PQCScriptsShortcuts::translateShortcut(QString combo) {
    ...
    if(combo.contains(" Button")) {
        const QString checkdir = parts.last();
        bool onlydir = true;
        for(const auto &d : checkdir) {
            if(d != 'N' && d != 'S' && d != 'E' && d != 'W') {
                onlydir = false;
                break;
            }
...
QString PQCScriptsShortcuts::translateMouseDirection(QStringList dirs) {
