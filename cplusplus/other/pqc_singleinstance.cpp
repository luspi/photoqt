...
bool PQCSingleInstance::importConfig() {

    // no import is to be done
...
// Read file - if something went wrong, output error message and stop here
    ...
    while(archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        if(allfiles.contains(filenameinside)) {
            ...
            if(size <= 0) {
                qWarning() << "Invalid size of file in archive:" << size;
                archive_read_close(a);
                archive_read_free(a);
                return false;
            }
            ...
            if(total != size) {
                qWarning() << QString("Failed to read file data, read size (%1) doesn't match expected size (%2).").arg(total).arg(size);
                archive_read_close(a);
                archive_read_free(a);
                return false;
            }
    ...
    // no matter what we remove this temporary file to not keep re-attempting importing it every time
