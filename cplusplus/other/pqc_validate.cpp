...
bool PQCValidate::validateShortcutsDatabase() {
    ...
    if(!query.exec("SELECT count() FROM PRAGMA_TABLE_INFO('shortcuts')")) {
        qWarning() << "Error checking for 'shortcuts' columns:" << query.lastError().text();
        query.clear();
        return false;
    }
    ...
    if(c == 0) {
        ...
        if(!query.exec("CREATE TABLE 'shortcuts' (`combo` TEXT UNIQUE,`commands` TEXT,`cycle` INTEGER,`cycletimeout` INTEGER,`simultaneous` INTEGER)")) {
            qCritical() << "ERROR: Unable to create new shortcuts table:" << query.lastError().text();
            query.clear();
            return false;
        }
        ...
        if(!query.exec("SELECT command,shortcuts FROM builtin")) {
            qWarning() << "ERROR: Unable to read old builtin data:" << query.lastError().text();
            query.clear();
            return false;
        }
        ...
        if(!query.exec("SELECT command,arguments,shortcuts,close FROM external")) {
            qWarning() << "ERROR: Unable to read old external data:" << query.lastError().text();
            query.clear();
            return false;
        }
        ...
        while(query.next()) {
            ...
            QString val = QString("%1:/:/:%2:/:/:%3").arg(cmd, args).arg(close);
        ...
        while (iter.hasNext()) {
            ...
            if(!query.exec()) {
                qWarning() << "ERROR: Unable to write new data:" << query.lastError().text();
                query.clear();
                return false;
            }
        ...
        if(dbinstalled.lastError().text().trimmed().length()) {
            qWarning() << "ERROR committing database:" << dbinstalled.lastError().text();
            return false;
        }
