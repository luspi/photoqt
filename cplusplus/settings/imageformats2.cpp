...
#include "imageformats2.h"

PQImageFormats2::PQImageFormats2() {
    ...
    // insert statements for all endings
    /*QString sql = "";
    QStringList duplicates;
    QStringList existing;
    for(int iCat = 0; iCat < categories.length(); ++iCat){

        qDebug() << categories[iCat];

        QString prevGrp = "";
        QString desc = "";
        QString endingStr = "";
        for(QString ending : setupAvailable[iCat].keys()) {
            if(existing.contains(ending)) {
                if(!duplicates.contains(ending))
                    duplicates << ending;
                continue;
            } else
                existing << ending;
            if(prevGrp != "" && prevGrp != setupAvailable[iCat][ending][0]) {
                sql += QString("INSERT INTO imageformats (endings, description) VALUES('%1', '%2'); ").arg(endingStr).arg(desc);
                endingStr = "";
            }
            prevGrp = setupAvailable[iCat][ending][0];
            desc = setupAvailable[iCat][ending][1];
            if(endingStr != "") endingStr += ",";
            endingStr += ending.remove(0,2);
        }

    }
    qDebug() << sql;
    qDebug();
    qDebug() << "duplicates:" << duplicates;*/
...
//        QString cat = categories[iCat];
//        if(cat == "raw") cat = "libraw";

//        for(QString ending : setupAvailable[iCat].keys()) {

//            QString o = ending;
//            QString e = ending.remove(0,2);

//            qDebug() << QString("UPDATE imageformats SET enabled=%1, %2=1 WHERE endings LIKE '%3';").arg(setupAvailable[iCat][o][2]).arg(cat).arg("%," + e + ",%");
//            qDebug() << QString("UPDATE imageformats SET enabled=%1, %2=1 WHERE endings LIKE '%3';").arg(setupAvailable[iCat][o][2]).arg(cat).arg(e + ",%");
//            qDebug() << QString("UPDATE imageformats SET enabled=%1, %2=1 WHERE endings LIKE '%3';").arg(setupAvailable[iCat][o][2]).arg(cat).arg("%," + e);
//            qDebug() << QString("UPDATE imageformats SET enabled=%1, %2=1 WHERE endings='%3';").arg(setupAvailable[iCat][o][2]).arg(cat).arg(e);
//        }
...
//    }
    ...
    connect(saveTimer, &QTimer::timeout, this, &PQImageFormats2::saveEnabledFormats);

    connect(this, &PQImageFormats2::enabledFileformatsQtChanged, this, [=](QStringList) {saveTimer->start();});

}


void PQImageFormats2::setEnabledFileformats(QString cat, QStringList val, bool withSaving) {

    DBG << CURDATE << "PQImageFormats2::setEnabledFileformats()" << NL
...
void PQImageFormats2::composeAvailableFormats() {

    DBG << CURDATE << "PQImageFormats2::composeAvailableFormats()" << NL;
...
void PQImageFormats2::composeEnabledFormats(bool withSaving) {

    DBG << CURDATE << "PQImageFormats2::composeEnabledFormats()" << NL
        << CURDATE << "** withSaving = " << withSaving << NL;
...
// Save file formats
void PQImageFormats2::saveEnabledFormats() {

    DBG << CURDATE << "PQImageFormats2::saveEnabledFormats()" << NL;
