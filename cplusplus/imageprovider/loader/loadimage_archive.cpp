...
QImage PLoadImage::Archive(QString filename, QSize maxSize) {
    ...
    while(archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        if(filenameinside == compressedFilename) {
            ...
            ssize_t r = archive_read_data(a, reinterpret_cast<void*>(buff), static_cast<size_t>(size));
            if(r != size) {
                std::stringstream ss;
            ...
            ret = QImage::fromData(buff, static_cast<int>(size));
