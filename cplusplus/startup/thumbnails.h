...
namespace StartupCheck {
	...
	namespace Thumbnails {
		...
		static inline void checkThumbnailsDatabase(int update, bool nothumbs, QString *settingsText, bool verbose) {
			...
			if(verbose) LOG << DATE << "StartupCheck::Thumbnails" << NL;
			...
			if(!database.exists()) {
				...
				if(verbose) LOG << DATE << "Create Thumbnail Database" << NL;

				QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE", "thumbDB1");
				db.setDatabaseName(CFG_THUMBNAILS_DB);
				if(!db.open()) LOG << DATE << "ERROR: Couldn't open thumbnail database:" << db.lastError().text().trimmed().toStdString() << NL;
				QSqlQuery query(db);
				...
				query.exec();
				if(query.lastError().text().trimmed().length()) LOG << DATE << "ERROR (Creating Thumbnail Datbase):" << query.lastError().text().trimmed().toStdString() << NL;
				query.clear();


			} else if(update != 0) {

				if(verbose) LOG << DATE << "Opening Thumbnail Database" << NL;
				...
				if(query_check.record().value(0) == 0) {
					...
					query.exec();
					if(query.lastError().text().trimmed().length()) LOG << DATE << "ERROR (Adding origwidth to Thumbnail Database):" << query.lastError().text().trimmed().toStdString() << NL;
					query.clear();
					query.prepare("ALTER TABLE Thumbnails ADD COLUMN origheight INT");
					query.exec();
					if(query.lastError().text().trimmed().length()) LOG << DATE << "ERROR (Adding origheight to Thumbnail Database):" << query.lastError().text().trimmed().toStdString() << NL;
					query.clear();
				}
				query_check.clear();
