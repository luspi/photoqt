...
PQCFileFolderModel::PQCFileFolderModel(QObject *parent) : QObject(parent) {
    ...
    connect(&PQCCSettings::get(), &PQCCSettings::imageviewSortImagesAscendingChanged, this, &PQCFileFolderModel::loadDataFileDialog);
    connect(&PQCCSettings::get(), &PQCCSettings::imageviewSortImagesAscendingChanged, this, &PQCFileFolderModel::loadDataMainView);
    connect(&PQCCSettings::get(), &PQCCSettings::imageviewSortImagesByChanged, this, &PQCFileFolderModel::loadDataFileDialog);
    connect(&PQCCSettings::get(), &PQCCSettings::imageviewSortImagesByChanged, this, &PQCFileFolderModel::loadDataMainView);
    connect(&PQCCSettings::get(), &PQCCSettings::filedialogShowHiddenFilesFoldersChanged, this, &PQCFileFolderModel::loadDataFileDialog);
    connect(&PQCCSettings::get(), &PQCCSettings::filedialogShowHiddenFilesFoldersChanged, this, &PQCFileFolderModel::loadDataMainView);
...
void PQCFileFolderModel::loadDataMainView() {
    ...
    if(info.absolutePath() == cacheAdvancedSortFolderName)
        ...
        advancedSortMainView(PQCCSettings::get().getImageviewAdvancedSortCriteria(),
                             PQCCSettings::get().getImageviewAdvancedSortAscending(),
                             PQCCSettings::get().getImageviewAdvancedSortQuality(),
                             PQCCSettings::get().getImageviewAdvancedSortDateCriteria());
...
QStringList PQCFileFolderModel::getAllFolders(QString folder, bool forceShowHidden) {
    ...
    const bool sortReversed = !PQCCSettings::get().getImageviewSortImagesAscending();
    QString sortBy = PQCCSettings::get().getImageviewSortImagesBy();
    const bool showHidden = (PQCCSettings::get().getFiledialogShowHiddenFilesFolders() || forceShowHidden);
...
QStringList PQCFileFolderModel::getAllFiles(QString folder, bool ignoreFiltersExceptDefault, bool enforceOnlyIncludingThisFolder) {
    ...
    const bool sortReversed = !PQCCSettings::get().getImageviewSortImagesAscending();
    QString sortBy = PQCCSettings::get().getImageviewSortImagesBy();
    const bool showHidden = PQCCSettings::get().getFiledialogShowHiddenFilesFolders();
                ...
                for(const auto &f : lst) {
                    ...
                    if(!ignoreFiltersExceptDefault) {
                        ...
                        if(!m_imageResolutionFilter.isNull()) {
                            ...
                            //TODO!!!
                            QSize origSize = QSize();//PQCLoadImage::get().load(f.absoluteFilePath());
                    ...
                    if((m_nameFilters.size() == 0 || (!ignoreFiltersExceptDefault && (m_nameFilters.contains(suffix1) || m_nameFilters.contains(suffix2)))) &&
                        ...
                        if(m_filenameFilters.length() == 0 || ignoreFiltersExceptDefault) {
                            // we need to exclude video files connected to Apple Live Videos (if support enabled)
                            if(PQCCSettings::get().getFiletypesLoadAppleLivePhotos() && suffix1 == "mov" && QFileInfo::exists(f.absolutePath()+"/"+f.baseName()+".heic"))
                                continue;
