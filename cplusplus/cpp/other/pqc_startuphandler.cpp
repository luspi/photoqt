...
PQCStartupHandler::PQCStartupHandler(QObject *parent) : QObject(parent) {
    ...
    connect(&PQCCLocalServer::get(), &PQCCLocalServer::performAction, this, [=](QString what, QStringList args) {
        if(what == "setupFresh")
            setupFresh();
    });
...
void PQCStartupHandler::exportData(QString path) {
    ...
    if(PQCCScriptsConfig::get().exportConfigTo(path))
...
void PQCStartupHandler::importData(QString path) {
    ...
    if(choice == "y" || choice == "yes") {
        ...
        if(PQCCScriptsConfig::get().importConfigFrom(path))
            std::cout << "done! Goodbye." << std::endl << std::endl;
        else
            std::cout << "failed! Goodbye." << std::endl << std::endl;
...
void PQCStartupHandler::showInfo() {
    ...
    std::cout << std::endl
              ...
              << std::endl << std::endl
              << PQCCScriptsConfig::get().getConfigInfo().toStdString()
              << std::endl;
