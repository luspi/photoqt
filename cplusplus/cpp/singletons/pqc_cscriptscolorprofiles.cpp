...
void PQCCScriptsColorProfiles::loadColorProfileInfo() {
...
QString PQCCScriptsColorProfiles::getColorProfileFor(QString path) {
...
QString PQCCScriptsColorProfiles::applyColorProfile(QString filename, QImage &img) {
    ...
    if(profileName.startsWith("::")) {
        ...
        QString prf = _applyColorSpaceQt(img, filename, sp);
        if(prf != "") {
            PQCCDbusServer::get().sendMessage("colorprofile", QString("%1\n%2").arg(filename, prf));
            return prf;
        } else
    ...
    if(!colorSpaceManuallySet && PQCCSettings::get().getImageviewColorSpaceLoadEmbedded()) {
    ...
    QString def = PQCCSettings::get().getImageviewColorSpaceDefault();
    if(def != "") {
...
QString PQCCScriptsColorProfiles::_applyColorSpaceQt(QImage &img, QString filename, QColorSpace sp) {
...
int PQCCScriptsColorProfiles::toLcmsFormat(QImage::Format fmt) {
...
QString PQCCScriptsColorProfiles::_applyColorSpaceLCMS2(QImage &img, QString filename, cmsHPROFILE targetProfile) {
