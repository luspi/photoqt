...
public:
    ...
    Q_INVOKABLE QString        getSuffixLowerCase(QString path) {
        return PQCScriptsFilesPaths::get().getSuffixLowerCase(path);
    }
    Q_INVOKABLE QString        getCompleteSuffix(QString path) {
        return PQCScriptsFilesPaths::get().getCompleteSuffix(path);
    }
    Q_INVOKABLE QString        getCompleteSuffixLowerCase(QString path) {
        return PQCScriptsFilesPaths::get().getCompleteSuffixLowerCase(path);
    }
    ...
    Q_INVOKABLE QString        getFilename(QString fullpath) {
        return PQCScriptsFilesPaths::get().getFilename(fullpath);
    }
    Q_INVOKABLE QString        getDir(QString fullpath) {
        return PQCScriptsFilesPaths::get().getDir(fullpath);
    }
    Q_INVOKABLE QString        getDirname(QString fullpath) {
        return PQCScriptsFilesPaths::get().getDirname(fullpath);
    }
    Q_INVOKABLE QString        getFullArchivePath(QString path) {
        return PQCScriptsFilesPaths::get().getFullArchivePath(path);
    }
    Q_INVOKABLE bool           isUrl(QString path) {
        return PQCScriptsFilesPaths::get().isUrl(path);
    }
    Q_INVOKABLE bool           areDirsTheSame(QString folder1, QString folder2) {
        return PQCScriptsFilesPaths::get().areDirsTheSame(folder1, folder2);
    }

    // folder methods
    Q_INVOKABLE bool        isFolder(QString path) {
        return PQCScriptsFilesPaths::get().isFolder(path);
    }
    Q_INVOKABLE QStringList getFoldersIn(QString path) {
        return PQCScriptsFilesPaths::get().getFoldersIn(path);
    }
    Q_INVOKABLE QString     goUpOneLevel(QString path) {
        return PQCScriptsFilesPaths::get().goUpOneLevel(path);
    }
    Q_INVOKABLE bool        isExcludeDirFromCaching(QString filename) {
        return PQCScriptsFilesPaths::get().isExcludeDirFromCaching(filename);
    }

    // file methods
    Q_INVOKABLE QDateTime getFileModified(QString path) {
        return PQCScriptsFilesPaths::get().getFileModified(path);
    }
    Q_INVOKABLE QString   getFileType(QString path) {
        return PQCScriptsFilesPaths::get().getFileType(path);
    }
    Q_INVOKABLE QString   getFileSizeHumanReadable(QString path) {
        return PQCScriptsFilesPaths::get().getFileSizeHumanReadable(path);
    }
    Q_INVOKABLE double    convertBytesToGB(const qint64 bytes) {
        return PQCScriptsFilesPaths::get().convertBytesToGB(bytes);
    }
    Q_INVOKABLE QString   createTooltipFilename(QString fname) {
        return PQCScriptsFilesPaths::get().createTooltipFilename(fname);
    }
    Q_INVOKABLE void      openInDefaultFileManager(QString filename) {
               PQCScriptsFilesPaths::get().openInDefaultFileManager(filename);
    }

    // folder and file methods
    Q_INVOKABLE bool doesItExist(QString path) {
        return PQCScriptsFilesPaths::get().doesItExist(path);
    }
    Q_INVOKABLE bool isOnNetwork(QString filename) {
        return PQCScriptsFilesPaths::get().isOnNetwork(filename);
    }

    // get some fixed directories
    Q_INVOKABLE QString getHomeDir() {
        return PQCScriptsFilesPaths::get().getHomeDir();
    }
    ...
    Q_INVOKABLE QString findNextcloudFolder() {
        return PQCScriptsFilesPaths::get().findNextcloudFolder();
    }
    Q_INVOKABLE QString findOwnCloudFolder() {
        return PQCScriptsFilesPaths::get().findOwnCloudFolder();
    }
